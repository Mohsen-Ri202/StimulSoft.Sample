using System.Diagnostics;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Data.SqlClient;
using Microsoft.EntityFrameworkCore;
using Stimulsoft.Report;
using Stimulsoft.Report.Mvc;
using StimulSoft.Sample.Models;

namespace StimulSoft.Sample.Controllers
{
    public class HomeController : Controller
    {
        private readonly ILogger<HomeController> _logger;
        private readonly NopTestContext _context;
        public HomeController(ILogger<HomeController> logger, NopTestContext context)
        {
            //StiLicense.Key="";
            //StiLicense.LoadFromFile("license.key");
            _logger = logger;
            _context = context;
        }

        public IActionResult Index()
        {
            var model = _context.Vendors.ToList();
            return View(model);
        }

        public IActionResult ShowReport()
        {
            return View();
        }

        public IActionResult Report()
        {
            StiReport report = new StiReport();
            report.Load("wwwroot/Report/Report.mrt");
            var model = _context.Vendors.ToList();
            report.RegData("rp", model);
            return StiNetCoreViewer.GetReportResult(this, report);
        }
        public IActionResult ViewEvent()
        {
            return StiNetCoreViewer.ViewerEventResult(this);
        }

        public IActionResult ShowVendor(int id)
        {
            VendorViewModel model = new VendorViewModel()
            {
                Id = id
            };
            return View(model);
        }
      
        public IActionResult VendorReport(int id)
        {
            var vendorId = new SqlParameter("@Id", id);
            StiReport report = new StiReport();
            report.Load("wwwroot/Report/Report2.mrt");

            //This parameter will initialize by EF Core
            //var model = _context.Vendors.FirstOrDefault(f=>f.Id==id);

            //This parameter will initialize by a stored procedure that get a parameter as ID
            var model = _context.Vendors.FromSqlRaw("exec VendorSP @Id", vendorId).ToList();
            report.RegData("vr", model);
            return StiNetCoreViewer.GetReportResult(this, report);
        }

        public IActionResult Privacy()
        {
            return View();
        }

        [ResponseCache(Duration = 0, Location = ResponseCacheLocation.None, NoStore = true)]
        public IActionResult Error()
        {
            return View(new ErrorViewModel { RequestId = Activity.Current?.Id ?? HttpContext.TraceIdentifier });
        }
    }
}